...
- `options.env` a JS expression that evaluates to the env object, or an object with `imports` (array of import statement strings) and `expression` (a JS expression), used to populate dynamic env vars before instrumentation runs.
